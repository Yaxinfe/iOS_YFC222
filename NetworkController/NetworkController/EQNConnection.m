...
#import "EQNData.h"
#import "EQNConnection.h"


// Typically, EQN_QUEUED_CONNECTIONS is set to 0 or 1 in the target's prefix header.
// Note that NSURLConnection setDelegateQueue appears to be broken on iOS 5.1, causing application-wide deadlocks.
// See README.md for details.

#if EQN_QUEUED_CONNECTIONS
...
NSString * const EQNConnectionActivityBegan = @"EQNConnectionActivityBegan";
NSString * const EQNConnectionActivityEnded = @"EQNConnectionActivityEnded";

NSString* stringForRequestMethod(EQNRequestMethod method) {
    switch (method) {
        case EQNRequestMethodGET:   return @"GET";
        case EQNRequestMethodPOST:  return @"POST";
        default:
...
- (NSURLRequest *)connection:(NSURLConnection *)connection
    ...
    EQNVerbose(@"%p: willSendRequest", self);
    return request;
}
...
//    EQNVerbose(@"%p: needNewBodyStream", self);
//}
...
- (BOOL)connectionShouldUseCredentialStorage:(NSURLConnection *)connection {
    ...
    EQNVerbose(@"%p: connectionShouldUseCredentialStorage", self);
...
willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    ...
    EQNVerbose(@"%p: willSendRequestForAuthenticationChallenge", self);
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSHTTPURLResponse *)response {
    ...
    EQNVerbose(@"%p: didReceiveResponse", self);
    EQNVerbose(@"Response Headers: %@", [response allHeaderFields]);
...
totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite {
    ...
    EQNVerbose(@"%p: didSendBodyData", self);
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    EQNVerbose(@"%p: didReceiveData", self);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    EQNVerbose(@"%p: didFinishLoading", self);
...
+ (void)cancelAllConnections {
    for (EQNConnection *c in [[self connections] allObjects]) {
        [c cancel];
    }
}
...
progressBlock:(EQNProgressBlock)progressBlock {
    ...
    EQNConnection *c = [EQNConnection new];
...
#if TARGET_OS_IPHONE
    c.shouldRunInBackground = (method == EQNRequestMethodPOST);
...
- (void)setMethod:(EQNRequestMethod)method {
    ASSERT_UNSTARTED;
    _method = method;
}
...
- (void)cleanup {
    ...
    if (requests.count == 0) {
        [[NSNotificationCenter defaultCenter] postNotificationName:EQNConnectionActivityEnded object:nil];
    }
}


- (void)cancelConnection {
    EQNVerbose(@"%p: cancelConnection", self);
...
- (void)performComplete {
    EQNVerbose(@"%p: performComplete", self);
    self.connection = nil;
...
- (EQNConnection *)start {
    ...
    EQNVerbose(@"%p: enqueue (#%lu)", self, (unsigned long)[[self.class connections] count]);
    ...
    if (connections.count == 1) {
        [[NSNotificationCenter defaultCenter] postNotificationName:EQNConnectionActivityBegan object:nil];
    }
    ...
    if (![NSURLConnection canHandleRequest:urlRequest]) {
        [self failWithError:
         [NSError errorWithDomain:EQNConnectionErrorDomain
                             code:0
...
- (void)didExpireInBackground {
    ...
    [self failWithError:
     [NSError errorWithDomain:EQNConnectionErrorDomain
                         code:EQNConnectionErrorCodeExpiredInBackgroundTask
...
- (NSString *)makeRequestString {
    ...
    if (self.method != EQNRequestMethodGET || !self.parameters.count) {
...
- (NSData*)makePostBodyWithBoundary:(NSString*)boundary {
    NSAssert1(self.method == EQNRequestMethodPOST, @"wrong method: %d", self.method);
    ...
    for (id key in self.parameters) {
        ...
        if (![key isKindOfClass:[NSString class]]) {
            EQNLogError(@"skipping bad parameter key: key class: %@ key: %@", [key class], key);
        ...
        else if ([val isKindOfClass:[EQNData class]]) {
            typeString  = [val mimeTypeString];
            valData     = [val data];
            fileName    = [val fileName];
        }
        else {
            EQNLogError(@"skipping bad POST parameter value: key: %@; value class: %@; value: %@",
                        key, [val class], val);
...
void logParameter(EQNRequestMethod method, id key, id val) {
    switch (method) {
#if !EQN_LOG_GET_PARAMETERS
        case EQNRequestMethodGET: break;
#endif
#if !EQN_LOG_POST_PARAMETERS
        case EQNRequestMethodPOST: break;
#endif
        default:
            EQNErr(@"  %-16s : %@", [[key description] UTF8String], val);
    }
}
...
#if EQN_LOG_REQUESTS
    EQNLog(@"%@: %@", stringForRequestMethod(self.method), requestString);
...
EQNErr(@"- header: %-16s : %@", [k UTF8String], v); [r setValue:v forHTTPHeaderField:k];
#else
#define SET_HEADER(k, v) [r setValue:v forHTTPHeaderField:k];
    ...
    switch (self.method) {
        ...
        case EQNRequestMethodGET:
            break; // no body
            
        case EQNRequestMethodPOST: {
            
            // decide if this request needs to be multipart
            BOOL multipart = NO;
            for (id k in self.parameters) {
                if ([[self.parameters valueForKey:k] isKindOfClass:[EQNData class]]) {
                    multipart = YES;
                    break;
                }
            };
            ...
            if (multipart) {
                ...
                NSString *boundary = @"--EQN-POST-boundary-wXzBVZAKUhpccuA9";
        ...
        default:
            [NSException raise:@"EQNConnectionException" format:@"unknown request method: %d", self.method];
    }
