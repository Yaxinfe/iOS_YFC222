...
#if EQN_LOG
#define EQNLog(...) NSLog(@"EQN: " __VA_ARGS__)
#define EQNErr(...) fprintf(stderr, "%s\n", [[NSString stringWithFormat:__VA_ARGS__] UTF8String])
#else
#define EQNLog(...) ((void)0)
#define EQNErr(...) ((void)0)
#endif

// internal verbose logging, conditionally enabled
#if EQN_LOG_VERBOSE
#define EQNVerbose(...) NSLog(@"EQNV: " __VA_ARGS__)
#else
#define EQNVerbose(...) ((void)0)
#endif

// always log errors
#define EQNLogError0(string)        NSLog(@"ERROR: %s: " string, __FUNCTION__)
#define EQNLogError(format, ...)    NSLog(@"ERROR: %s: " format, __FUNCTION__, __VA_ARGS__)
...
typedef enum {
    EQNHTTPCodeClassUnknown     = 0,
    EQNHTTPCodeClassInfo        = 1, // 1xx: Informational
    EQNHTTPCodeClassSuccess     = 2, // 2xx: Successful
    EQNHTTPCodeClassRedirect    = 3, // 3xx: Redirection
    EQNHTTPCodeClassError       = 4, // 4xx: Error
    EQNHTTPCodeClassServerError = 5, // 5xx: Server Error
} EQNHTTPCodeClass;


// error codes for EQNConnectionErrorDomain
typedef enum {
    EQNConnectionErrorCodeUnknown = 0,
    EQNConnectionErrorCodeJSONResultType,          // JSON parsing returned a root object of unexpected type, e.g. array instead of dict
    EQNConnectionErrorCodeExpiredInBackgroundTask, // OS expired the background task for the request
} EQNConnectionErrorCode;


extern NSString * const EQNConnectionErrorDomain;


BOOL httpCodeIsOfClass(int httpCode, EQNHTTPCodeClass httpClass);


@interface NSDictionary (EQN)
...
@interface NSDate (EQN)

+ (NSTimeInterval)posixTime;

@end

@interface NSArray (EQN)

- (NSString *)urlEncodedString;

@end

@interface NSString (EQN)
