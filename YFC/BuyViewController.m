...
@interface BuyViewController ()<UITableViewDataSource, UITableViewDelegate, UIAlertViewDelegate>
{
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < selectedArray.count; i++) {
        NSString *ballNumber = [[selectedArray objectAtIndex:i] objectForKey:@"price"];
        NSString *count = [[selectedArray objectAtIndex:i] objectForKey:@"number"];
        
        totalCount = totalCount + [ballNumber intValue] * [count intValue];
    }
...
//    movePriceLbl.text = @"运费";
    ...
    [buyBtn addTarget:self action:@selector(basketBuyBtnClicked) forControlEvents:UIControlEventTouchUpInside];
...
- (void)selectDelegate{
    ...
    for (int i = 0; i < selectedArray.count; i++) {
        NSString *initfare = [[selectedArray objectAtIndex:i] objectForKey:@"initfare"];
        NSString *wrate = [[selectedArray objectAtIndex:i] objectForKey:@"wrate"];
        NSString *weight = [[selectedArray objectAtIndex:i] objectForKey:@"weight"];
        NSString *brate = [[selectedArray objectAtIndex:i] objectForKey:@"brate"];
        NSString *bulk = [[selectedArray objectAtIndex:i] objectForKey:@"bulk"];
        NSString *drate = [[selectedArray objectAtIndex:i] objectForKey:@"drate"];
        NSString *number = [[selectedArray objectAtIndex:i] objectForKey:@"number"];
        
        moveCount = moveCount + [initfare intValue] + round([wrate floatValue]*[weight floatValue] + [brate floatValue]*[bulk floatValue] + [drate floatValue]*[strDistance floatValue])*[number intValue];
    }
    movePrice_Lbl.text = [NSString stringWithFormat:@"%d", moveCount];
    countLbl.text = [NSString stringWithFormat:@"%d", [price_Lbl.text intValue] + [movePrice_Lbl.text intValue]];
}

-(void)basketBuyBtnClicked{
    NSLog(@"%@", sendAddressLbl.text);
    if (sendNameLbl.text != nil && sendPhoneLbl.text != nil && sendAddressLbl.text != nil) {
        NSMutableArray *jsonArray = [[NSMutableArray alloc] init];
        
        for (int i = 0; i < selectedArray.count; i++) {
            NSDictionary *parameter = @{@"detailpid":[[selectedArray objectAtIndex:i] objectForKey:@"detailpid"],
                                        @"number":[[selectedArray objectAtIndex:i] objectForKey:@"number"],
                                        @"ballnumber":[[selectedArray objectAtIndex:i] objectForKey:@"price"],
                                        @"cartid":[[selectedArray objectAtIndex:i] objectForKey:@"cartid"],
                                        @"itemid":[[selectedArray objectAtIndex:i] objectForKey:@"itemid"]
                                        };
            [jsonArray addObject:parameter];
        }
        
        NSData *jsonData_CartList = [NSJSONSerialization dataWithJSONObject:jsonArray options:NSJSONWritingPrettyPrinted error:nil];
        NSString *jsonString = [[NSString alloc] initWithData:jsonData_CartList encoding:NSUTF8StringEncoding];
...
//        [[GlobalPool sharedInstance].OAuth2Manager.requestSerializer setAuthorizationHeaderFieldWithCredential:[GlobalPool sharedInstance].credential];
        [[GlobalPool sharedInstance].OAuth2Manager POST:LC_SHOP_BUY_CARTLIST parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if ([status intValue] == 200) {
                NSString *applang = [[NSUserDefaults standardUserDefaults] objectForKey:@"applanguage"];
                if ([applang isEqualToString:@"ko"]) {
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"상품을 구매하였습니다." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                    [alert show];
                }
                if ([applang isEqualToString:@"cn"]) {
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"购买成功" delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil, nil];
                    [alert show];
                }
                ...
                NSString *applang = [[NSUserDefaults standardUserDefaults] objectForKey:@"applanguage"];
                if ([applang isEqualToString:@"ko"]) {
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"상품이 부족합니다." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                    [alert show];
                }
                if ([applang isEqualToString:@"cn"]) {
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"库存不足" delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil, nil];
                    [alert show];
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [lbl_Title setFrame:CGRectMake(120, 10, self.view.frame.size.width - 150, 40)];
    ...
    [lbl_Color setFrame:CGRectMake(120, 60, self.view.frame.size.width - 150, 20)];
    ...
    [lbl_Count setFrame:CGRectMake(self.view.frame.size.width - 60, 90, 50, 20)];
    ...
    lbl_Title.text = [[selectedArray objectAtIndex:indexPath.row] objectForKey:@"title"];
    lbl_Color.text = [[selectedArray objectAtIndex:indexPath.row] objectForKey:@"color"];
    lbl_BallCount.text = [[selectedArray objectAtIndex:indexPath.row] objectForKey:@"price"];
    lbl_Count.text = [[selectedArray objectAtIndex:indexPath.row] objectForKey:@"number"];
    
    NSString *path = [[selectedArray objectAtIndex:indexPath.row] objectForKey:@"mainurl"];
